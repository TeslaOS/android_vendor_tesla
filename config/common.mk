...
# CMSDK
include vendor/tesla/config/cmsdk_common.mk
...
ifeq ($(TESLA_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        ...
        ifeq ($(TARGET_BUILD_VARIANT),user)
            ifeq ($(TESLA_VERSION_MAINTENANCE),0)
                TESLA_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(TESLA_BUILD)
            else
                TESLA_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(TESLA_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(TESLA_BUILD)
            endif
        else
            TESLA_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(TESLA_BUILD)
        endif
    ...
    ifeq ($(TESLA_VERSION_MAINTENANCE),0)
        ...
        TESLA_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(TESLA_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-$(TESLA_BUILDTYPE)$(TESLA_EXTRAVERSION)-$(TESLA_BUILD)
    endif
endif
  ...
  ifneq ($(TESLA_BUILDTYPE), UNOFFICIAL)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
      ...
      TARGET_VENDOR_RELEASE_BUILD_ID := $(TARGET_VENDOR_RELEASE_BUILD_ID)
		...
		ifeq ($(TESLA_VERSION_MAINTENANCE),0)
        TESLA_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(TESLA_BUILD)
    else
        TESLA_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(TESLA_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(TESLA_BUILD)
    endif
endif
endif
endif
